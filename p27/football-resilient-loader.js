(function(){
    ...
    try{
      ...
      for(const ev of (j?.events||[]))for(const c of (ev?.competitions||[])){
        ...
        for(const f of fixtures){
          ...
          const st=finalLocked&&incoming!=='post'?'post':incoming;
          ...
          if(score!==null&&(st==='in'||st==='post')){
            f._atlasLiveScore=score;
            f.s=score;
          }
          if(st==='post'){
  ...
  async function load(){
    ...
    window.ATLAS_FOOT_LIVE_STATUS={ok:providersOk>0,updatedAt:new Date().toISOString(),providersOk,totalEvents,matchesUpdated,source:'ATLAS same-origin broker · resilient loader',rules:{threeStatesOnly:true,scoreAloneNeverFinal:true,liveScoreVisible:true,finalLock:true}};
